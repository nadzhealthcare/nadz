...
export function NadPlusIvTherapyClient({ data }) {
  return (
    <>
      ...
      <>
                ...
                <div className="relative w-full">
                  <div className="rounded-3xl overflow-hidden bg-white border border-gray-200 shadow-lg aspect-[4/3]">
                    <img
                      ...
                      alt={data.whatIs?.imageAlt || 'NAD+ IV Therapy'}
                    ...
                    {items.map((item, index) => (
                      <div key={index} className={`bg-white rounded-[14px] border border-gray-200 shadow-sm hover:shadow-lg hover:border-primary-mediumBlue/40 p-4 md:p-5 transition-all duration-300 group${n === 1 ? ' w-full max-w-2xl' : ''}`}>
                        <div className="flex items-start gap-3">
                          ...
                          <span className="text-base md:text-lg text-text-gray leading-relaxed">{item}</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {data.howItWorks && (data.howItWorks.title || data.howItWorks.paragraph || (data.howItWorks.bullets?.length > 0)) && (
          <section className="relative py-20 md:py-28 lg:py-32 bg-white overflow-hidden">
            <div className="container mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
              <div className="max-w-4xl mx-auto">
                {data.howItWorks.title && (
                  <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-[1.1] sm:leading-tight tracking-tight mb-6 md:mb-8 text-center">
                    <span className="bg-gradient-to-r from-primary-heading via-primary-heading to-primary-heading bg-clip-text text-transparent">
                      {data.howItWorks.title}
                    </span>
                  </h2>
                )}
                {data.howItWorks.paragraph && (
                  <p className="text-base md:text-lg text-text-gray leading-relaxed mb-6">{data.howItWorks.paragraph}</p>
                )}
                {data.howItWorks.subtitle && (
                  <p className="text-sm md:text-base text-text-gray mb-6">{data.howItWorks.subtitle}</p>
                )}
                {data.howItWorks.bullets?.length > 0 && (
                  <ul className="space-y-3 mb-6">
                    {data.howItWorks.bullets.map((item, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <MdCheckCircle className="text-primary-mediumBlue flex-shrink-0 mt-0.5" />
                        <span className="text-text-gray">{item}</span>
                      </li>
                    ))}
                  </ul>
                )}
                {data.howItWorks.footer && (
                  <p className="text-text-gray text-base md:text-lg">{data.howItWorks.footer}</p>
                )}
              </div>
            </div>
          </section>
        ...
        {/* Conditions Section - from CMS */}
        {data.conditions && (data.conditions.title || (data.conditions.sections?.length > 0)) && (
          <section className="relative py-20 md:py-28 lg:py-32 bg-gray-50 overflow-hidden">
            <div className="container mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
              {data.conditions.title && (
                <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-[1.1] sm:leading-tight tracking-tight mb-10 md:mb-12 text-center">
                  <span className="bg-gradient-to-r from-primary-heading via-primary-heading to-primary-heading bg-clip-text text-transparent">
                    {data.conditions.title}
                  </span>
                </h2>
              ...
              <div className="space-y-8 md:space-y-10">
                {data.conditions.sections?.map((sec, i) => (
                  <div key={i} className="bg-white rounded-[14px] border border-gray-200 p-6 md:p-8 shadow-sm">
                    {sec.title && (
                      <h3 className="text-lg md:text-xl font-bold text-primary-heading mb-4">{sec.title}</h3>
                    )}
                    {sec.items?.length > 0 && (
                      <ul className="space-y-2">
                        {sec.items.map((item, j) => (
                          <li key={j} className="flex items-start gap-2">
                            <span className="text-primary-mediumBlue">•</span>
                            <span className="text-text-gray">{item}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                    {sec.note && <p className="text-sm text-text-gray mt-4 italic">{sec.note}</p>}
                  </div>
                ))}
              </div>
              {data.conditions.footer && (
                <p className="text-center text-text-gray mt-8 md:mt-10 text-base md:text-lg max-w-3xl mx-auto">
                  {data.conditions.footer}
                </p>
              )}
            </div>
        ...
        {/* Patient Reports Section - from CMS */}
        {data.patientReports && (data.patientReports.title || (data.patientReports.items?.length > 0)) && (
          <section className="relative py-20 md:py-28 lg:py-32 bg-white overflow-hidden">
            <div className="container mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
              {data.patientReports.title && (
                <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-[1.1] sm:leading-tight tracking-tight mb-8 md:mb-10 text-center">
                  <span className="bg-gradient-to-r from-primary-heading via-primary-heading to-primary-heading bg-clip-text text-transparent">
                    {data.patientReports.title}
                  </span>
                </h2>
              )}
              {data.patientReports.items?.length > 0 && (
                <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {data.patientReports.items.map((item, i) => (
                    <div key={i} className="flex items-start gap-3 p-4 bg-gray-50 rounded-[14px] border border-gray-200">
                      <MdCheckCircle className="text-primary-mediumBlue flex-shrink-0 mt-0.5" />
                      <span className="text-text-gray">{item}</span>
                    </div>
                  ))}
        ...
        {/* Journey Section - only when CMS has journey data */}
        {data.journey && (data.journey.title || (data.journey.steps?.length > 0)) && (
        <section className="relative py-20 md:py-28 lg:py-32 bg-white overflow-hidden">
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
        ...
        {data.faqs && (Array.isArray(data.faqs) ? data.faqs.length > 0 : (data.faqs?.items && data.faqs.items.length > 0)) && (
          <Faq
            title={data.faqs?.title || "FAQs – NAD+ IV Therapy in Dubai"}
