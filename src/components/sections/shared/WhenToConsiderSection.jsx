...
export const WhenToConsiderSection = ({
  title = "When to Consider IV Hydration Therapy",
  items = [],
  note,
}) => {
  ...
  const gridClass =
    n === 1
      ? "grid grid-cols-1 gap-4 sm:gap-6 justify-items-center"
      : "grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6";
  const cardBaseClass =
    "flex items-start gap-3 sm:gap-4 p-4 sm:p-6 bg-white rounded-[14px] sm:rounded-[14px] shadow-sm border border-gray-200/50 hover:shadow-lg hover:border-primary-mediumBlue/30 transition-all duration-300 group";
  const cardWidthClass = n === 1 ? " w-full max-w-2xl" : "";
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 md:px-8 relative z-10">
        ...
        >
            ...
            <div className={gridClass}>
              {items.map((item, index) => (
                <MotionDiv
                  ...
                  className={`${cardBaseClass}${cardWidthClass}`}
                >
          ...
          {note && (
            ...
            >
              <div className="bg-amber-50/80 border-l-4 border-amber-400 rounded-r-lg p-4 sm:p-6">
